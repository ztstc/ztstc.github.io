...
**结果演示:**![02_URDF文件执行rviz配置02.png](./1102_URDF文件执行rviz配置02.png)
...
rviz 启动后，会发现并没有盒装的机器人模型，这是因为默认情况下没有添加机器人显示组件，需要手动添加，添加方式如下:![01_URDF文件执行rviz配置01.png](./1101_URDF文件执行rviz配置01.png)![02_URDF文件执行rviz配置02.png](./1102_URDF文件执行rviz配置02.png)设置完毕后，可以正常显示了
...
首先，将当前配置保存进`config`目录![10_rviz配置保存.png](./1110_rviz配置保存.png)然后，`launch`文件中 Rviz 的启动配置添加参数:`args`,值设置为`-d 配置文件路径`
